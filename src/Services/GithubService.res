...
module CrossFetch = {
  @module("cross-fetch")
  external fetch: string => Js.Promise.t<Fetch.Response.t> = "fetch"
}
...
let validateUserPreviewList = userPreviewList => {
  ...
  | Some(dict) => {
      ...
      items: Belt.Array.map(
        decodeDictField(dict, "items", Js.Json.decodeArray),
        validateUserPreview,
      ),
    }
  | None => Js.Exn.raiseError("wasdwasd")
...
let getUserProfile = async username => {
  let response = await CrossFetch.fetch(`${baseUrl}/users/${username}`)
