    ...
    switch context.previews {
        | Some(previews) => {
            ...
            <Row className="mt-3 gap-3 justify-end">
                <button
                    className={`rounded-full border shadow px-3 py-1 aspect-square ${context.page == 1? "bg-stone-100" : ""} hover:bg-stone-200`}
                ...
                <span className="grid place-items-center w-10 rounded-full border shadow px-3 py-1 aspect-square text-lg">
                    {context.page->React.int}
                </span>
                <button
                    className={`rounded-full border shadow px-3 py-1 aspect-square ${context.page == pagesCount? "bg-stone-100" : ""} hover:bg-stone-200`}
                    onClick={_ => context.setPage((page) => page + 1)}
